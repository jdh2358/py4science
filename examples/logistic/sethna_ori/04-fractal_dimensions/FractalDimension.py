...
def DimensionEstimates(mu, epsilonList, nSampleMax):
    ...
    for i in range(len(epsilonList)-1):
        ...
        D_capacity_estimate = -(log(Nboxes[i+1])-log(Nboxes[i])) \
				/ (log(epsiP1)-log(epsi))
	D_capacity.append(D_capacity_estimate)
        D_inf_estimate = -(S0[i+1]-S0[i]) / (log(epsiP1)-log(epsi))
...
def PlotDimensionEstimates(mu, nSampleMax = 2**18, \
    ...
    matplot.plot(epsilon, D_capacity, 'bo', label=str("capacity %s"%mu))
    matplot.plot(epsilon, D_inf, 'ro', label=str("info %s"%mu))
    ...
    matplot.plot(epsilon, capacity_theory, 'b-', label='capacity theory')
    matplot.plot(epsilon, information_theory, 'r-', label='information theory')
    matplot.semilogx()
    matplot.legend()
    matplot.show()
